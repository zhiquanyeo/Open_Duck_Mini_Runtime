...
right eye, left eye, projector. Each segment's pixel count is
...
class LedController:
    ...
    def __init__(self, led_counts: Optional[Dict[str, int]] = None) -> None:
        ...
        self._right_slice = slice(0, n_right)
        self._left_slice = slice(n_right, n_right + n_left)
        self._proj_slice = slice(n_right + n_left, n_right + n_left + n_proj)
